...
function resolveWsUrl(opts: YoloInitOptions): string {
  ...
  // Default: same origin as the page. Next dev rewrites /ws/yolo to the
  // Python backend (see next.config.mjs), so a phone or tunnel can hit one
  // origin without thinking about ports. Coerce http(s) → ws(s).
  if (typeof window !== "undefined") {
    const proto = window.location.protocol === "https:" ? "wss:" : "ws:";
    const host = window.location.host || "localhost:3000";
    return `${proto}//${host}/ws/yolo`;
  }
  return "ws://localhost:3000/ws/yolo";
}
...
async function connect(opts: YoloInitOptions): Promise<void> {
  ...
  // attached.
  teardownSocket();
  const url = opts.serverUrl ? opts.serverUrl : (wsUrl ?? resolveWsUrl(opts));
  wsUrl = url;
  setStatus({
    stage: "downloading",
    backend: null,
